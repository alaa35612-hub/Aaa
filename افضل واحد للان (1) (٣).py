...
def _collect_immediate_events(
    ...
    for event in events:
        ...
        message = (
            f"تنبيه فوري: {ANSI_BOLD}{_format_symbol(event['symbol'])}{ANSI_RESET} "
            f"({timeframe}) - {label}: {event['display']}"
        )
        if event["time_display"] and event["time_display"] != "—":
            message += f" | {event['time_display']}"
        print(_colorize_directional_text(message, direction=event["direction"]), flush=True)
    return events
