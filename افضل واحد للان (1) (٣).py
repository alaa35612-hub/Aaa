...
class ConsoleInputs:
    max_age_bars: int = 1
    output_mode: str = "summary"  # {summary, alerts_only}
...
def _send_tg(cfg: Any, lines: Sequence[str]) -> None:
...
def print_symbol_summary(
    ...
    console_cfg: Optional[ConsoleInputs] = None,
) -> None:
    ...
    if console_cfg is not None:
        ...
        if isinstance(console_cfg, dict):
            mode_value = console_cfg.get("output_mode", "summary")
        else:
            mode_value = getattr(console_cfg, "output_mode", "summary")
        if isinstance(mode_value, str) and mode_value.strip():
            mode = mode_value.strip().lower()

    if mode in {"alerts_only", "immediate", "events_only"}:
        immediate_alerts = metrics.get("immediate_event_alerts") or []
        if not immediate_alerts:
            return
        header = (
            f"{header_color}{ANSI_BOLD}{symbol_display}{ANSI_RESET}"
            f" {ANSI_DIM}({timeframe}){ANSI_RESET}"
        )
        print(
            f"{header} — {ANSI_BOLD}تنبيهات فورية محدثة{ANSI_RESET}",
            flush=True,
        )
        for event in immediate_alerts:
            label = event.get("event_label") or event.get("event_key") or "حدث"
            display_raw = event.get("display") or "—"
            ...
            time_display = event.get("time_display")
            if isinstance(time_display, str) and time_display.strip() and time_display != "—":
                colored_display = (
                    f"{colored_display}{ANSI_DIM} | {time_display.strip()}{ANSI_RESET}"
                )
            price_payload = event.get("price")
            if isinstance(price_payload, tuple) and len(price_payload) == 2:
                price_text = f"{format_price(price_payload[0])} → {format_price(price_payload[1])}"
            elif isinstance(price_payload, (int, float)):
                price_text = format_price(price_payload)
            else:
                price_text = None
            if price_text:
                colored_display = (
                    f"{colored_display}{ANSI_DIM} | {price_text}{ANSI_RESET}"
                )
            print(
                f"  • {ANSI_LABEL}{label}{ANSI_RESET}: {colored_display}",
                flush=True,
            )
        print(f"{ANSI_DIM}{'-'*32}{ANSI_RESET}", flush=True)
        return
    ...
    *,
    ...
    async def _scan_once_async() -> Tuple[SmartMoneyAlgoProE5, List[Dict[str, Any]]]:
        ...
        async def process_symbol(index: int, symbol: str) -> None:
            ...
            console_cfg = getattr(runtime.inputs, "console", None)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        choices=["once", "stream"],
        default="stream",
        help="الوضع الافتراضي 'stream' يشغّل الماسح المتوازي المستمر؛ اختر 'once' للمسح المفرد",
    )
    parser.add_argument(
        "--scan-poll",
        type=float,
        default=2.0,
        help="الفاصل بالثواني بين دورات التحديث عند البث المباشر",
    )
    parser.add_argument(
        "--scan-duration",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="عدد الدورات لكل رمز عند البث المباشر (0 أو أقل يعني بلا حد)",
    )
    parser.add_argument(
    ...
    indicator_inputs.scanner_runtime.parallel_tasks = max(1, args.concurrency)
...
def _android_cli_entry() -> int:
    ...
    inputs.console.output_mode = "alerts_only"
